...
void SaveImage(Arena::IImage* pImage)
{
	// Convert image
	...
	//    inputs. Notice that an image's size and stride use bytes as a unit while
	//    the bits per pixel uses bits. 
	...
	//    operator (<<) triggers a save. Notice that the << operator accepts image
	//    data as constant unsigned 8-bit integer data (const uint8_t*) and file
	//    names as a character string (const char*). 
