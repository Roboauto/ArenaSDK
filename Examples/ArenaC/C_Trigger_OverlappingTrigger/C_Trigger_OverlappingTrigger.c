...
AC_ERROR OverlapTriggerOnExposureEndEvent(acDevice hDevice)
{
	...
	err = acNodeMapGetEnumerationValue(hNodeMap, "TriggerMode", triggerModeInitial, &triggerModeBufLen) |
		  acNodeMapGetEnumerationValue(hNodeMap, "TriggerSource", triggerSourceInitial, &triggerSourceBufLen) |
		  acNodeMapGetEnumerationValue(hNodeMap, "TriggerSelector", triggerSelectorInitial, &triggerSelectorBufLen) |
	...
	err = acNodeMapSetEnumerationValue(hNodeMap, "EventNotification", eventNotificationInitial) |
		  ...
		  acNodeMapSetEnumerationValue(hNodeMap, "ExposureAuto", exposureAutoInitial);
	if (err != AC_ERR_SUCCESS)
