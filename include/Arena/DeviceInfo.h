...
namespace Arena
{
	...
	public:
		 ...
		 *
		 * Model names are used to differentiate between products. Below are a few
		 * examples of Lucid Vision model names: 
		 *  - PHX050S-MC 
		 *  - PHX032S-CC 
		 *  - TRI032S-MC
		 *
		 * The model name returned by this getter is the same as the one received in
		 ...
		 *
		 * <B> VendorName </B> gets the vendor/manufacturer name of a device. Vendor
		 * names differentiate between device vendors/manufacturers. Lucid devices
		 * return 'Lucid Vision Labs'.
		 *
		 * The vendor name returned by this getter is the same as the one received in
		 * the GigE Vision discovery acknowledgement.
		 */
		 ...
		 *
		 * <B> SerialNumber </B> gets the serial number of a device. A serial number
		 * differentiates between devices. Each Lucid device has a unique serial
		 * number. Lucid serial numbers are numeric, but the serial numbers of other
		 * vendors may be alphanumeric.
		 *
		 * The serial number returned by this getter is the same as the one received
		 ...
		 *  - The default gateway
		 *
		 * <B> DefaultGateway </B> retrieves the default gateway of the device as a
		 * string.
		 */
		 ...
		 * supported, it is a customizable string with a maximum of 16 bytes that can
		 * be used to identify a device (Arena::IDevice::GetNodeMap, 'DeviceUserID').
		 *
		 ...
		 *  - Otherwise, false
		 *
		 * <B> IsDHCPConfigurationEnabled </B> retrieves whether DHCP is enabled on
		 * the device.
		 */
		 ...
		 *  - Otherwise, false
		 *
		 * <B> IsPersistentIpConfigurationEnabled </B> retrieves whether persistent
		 * IP is enabled on the device.
		 */
		 ...
		 *  - Otherwise, false
		 *
		 * <B> IsLLAConfigurationEnabled </B> retrieves whether LLA is enabled on the
		 * device.
		 */
		 ...
		 * running on the device. For Lucid devices, this refers to firmware version.
		 */
