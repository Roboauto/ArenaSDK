...
namespace Arena
{
	...
	public:
		 ...
		 * If the created from parameters, the offset X will be set to 0, no matter
		 * its original value.
		 *
		 * @warning 
		 *  - Causes undefined behaviour if buffer requeued 
		 ...
		 *
		 * <B> GetTimestamp </B> gets the timestamp of the image in milliseconds.
		 * Images are self-describing, so the device does not need to be queried to
		 * get this information.
		 *
		 * Images are either retrieved from a device (Arena::IDevice) or created by
		 ...
		 * no matter its original value.
		 *
		 * If more granularity is needed, the timestamp can also be retrieved in
		 * nanoseconds (Arena::IImage::GetTimestampNs).
		 *
		 * @warning 
		 *  - Causes undefined behaviour if buffer requeued 
		 *  - Properties lazily instantiated if buffer retrieved from device 
		 ...
		 * no matter its original value.
		 *
		 * If less granularity is needed, the timestamp can also be retrieved in
		 * milliseconds (Arena::IImage::GetTimestamp).
		 *
		 * @warning 
		 *  - Causes undefined behaviour if buffer requeued 
		 *  - Properties lazily instantiated if buffer retrieved from device 
