 ...
 *
 * <B> acDeviceStartStream </B> causes the device to begin streaming
 * image/chunk data (acBuffer). It must be called before image or chunk data
 * buffers are retrieved (acDeviceGetBuffer). The stream must be stopped
 ...
 *  - Returns AC_ERR_SUCCESS (0) on success
 *
 * <B> acDeviceInitializeEvents </B> causes the underlying events engine to
 * start listening for events. It must be called before waiting on events
 ...
 * data. It must be called after the event infrastructure has been initialized
 ...
 * device node map (acNodeMap), used to access a subset of cached device
 * related nodes (acNode).
 *
 * @see 
 *  - acNodeMap 
 *  - acNode
 ...
 *  - Returns AC_ERR_SUCCESS (0) on success
 *
 * <B> acDeviceGetTLInterfaceNodeMap </B> retrieves the already initialized
 * GenTL interface node map (acNodeMap), used to access interface related nodes
 * (acNode).
 *
 * @see 
 *  - acNodeMap 
