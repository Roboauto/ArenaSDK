 ...
 *  - Returns AC_ERR_SUCCESS (0) on success
 *
 * <B> acImageFactoryConvert </B> converts an image (acBuffer) to a select
 * pixel format. In doing so, it creates a completely new image, similar to a
 * deep copy but with a different pixel format. Images created with the image
