 ...
 * excluding transport layer protocol leaders. The payload data may include
 * image data, chunk data, or both.
 */
 ...
 *  - Returns AC_ERR_SUCCESS (0) on success
 *
 * <B> acBufferIsIncomplete </B> returns whether or not the payload is
 * complete.
 */
 ...
 *  - Returns AC_ERR_SUCCESS (0) on success
 *
 * <B> acBufferVerifyCRC </B> calculates the CRC of a buffer's data and
 * verifies it against the CRC value sent from the device via chunk data. This
 * helps verify that no data has been changed or missed during a transmission.
 * This function calls a global helper function to calculate the CRC
 * (acCalculateCRC32).
 *
 * @see 
 *  - acCalculateCRC
